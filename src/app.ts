...
import { ready as dbReady } from "@/utils/db";

const app = express();
const server = http.createServer(app);

function ensureLogDir() {
  const logDir = u.getPath(["logs"]);
...
function appendErrorLog(error: unknown) {
  try {
    ...
    const message = error instanceof Error ? error.stack || error.message : JSON.stringify(error, null, 2);
    fs.appendFileSync(path.join(logDir, "error.log"), `[${new Date().toISOString()}] ${message}\n`);
  } catch {
    // Error logging must not interrupt request handling.
  }
}
...
export default async function startServe(randomPort: Boolean = false) {
  ...
  if (process.env.NODE_ENV === "dev") {
    ...
    accessLogStream.on("error", appendErrorLog);
  ...
  app.use(
    ...
    (req, res, next) => {
      ...
      if (req.query.size) {
        ...
        const originalPath = path.resolve(ossDir, `.${req.path}`);
        if (!isPathInside(originalPath, ossDir) && originalPath !== ossDir) return res.status(400).end();
        ...
        const smallImagePath = path.resolve(smallImageBaseDir, `.${path.join(dir, `${base}_${sizeSubDir}${ext}`)}`);
        if (!isPathInside(smallImagePath, smallImageBaseDir) && smallImagePath !== smallImageBaseDir) return res.status(400).end();

        ensureThumbnail(originalPath, smallImagePath, sizeOpts)
          .then((thumbnailPath) => {
            if (thumbnailPath) {
          ...
          .catch(() => express.static(ossDir, { acceptRanges: false })(req, res, next));
        return;
      }
      next();
    },
