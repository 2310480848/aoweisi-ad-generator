...
export default router.post(
  ...
  async (req, res) => {
    const { key } = req.body;
    const vendorId = "lingkeai";
    const scriptModel = "claude-sonnet-4-6";
    const universalModel = "claude-haiku-4-5-20251001";
    ...
    try {
      const resText = await u.Ai.Text(`${vendorId}:${universalModel}`).invoke({
        prompt: "1+1等于几？,请直接回答2，不要解释",
      });
      if (resText.text) {
        await u.db("o_agentDeploy").where("key", "scriptAgent").update({
          model: scriptModel,
          modelName: `${vendorId}:${scriptModel}`,
          vendorId,
        });
        ...
        await u.db("o_agentDeploy").where("key", "universalAi").update({
          ...
          modelName: `${vendorId}:${universalModel}`,
          vendorId,
        });
