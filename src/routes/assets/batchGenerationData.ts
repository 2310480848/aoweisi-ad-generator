...
import { getAssetQueryTypes } from "@/utils/assetTypes";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const queryTypes = getAssetQueryTypes(type);
    const offset = (page - 1) * limit;
    let query = u.db("o_assets").select("*").where("projectId", projectId).whereIn("type", queryTypes);
    if (name) {
