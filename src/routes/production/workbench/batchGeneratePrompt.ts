...
export default router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const tasks = trackData.map((track: { trackId: number; info: { id?: number | null; sources: string; path?: string; src?: string; fileType?: string; prompt?: string | null }[] }) =>
        limit(async () => {
          // 查询参数
          const images = await Promise.all(
            track.info.map(async (item) => {
              ...
              if (item.sources === "upload") {
                return {
                  id: item.path || item.src || "upload",
                  type: item.fileType || "image",
                  name: item.prompt || "本地上传参考",
