...
const adAssetTypes = ["product", "person", "scene", "prop"] as const;
const assetExtractionInstruction =
  "\n\nExtract ad assets from each ad script. Use only these asset types: product/person/scene/prop. product = advertised product or SKU; person = model, spokesperson, customer, hand actor, or other person subject; scene = reusable location or environment; prop = supporting object that is not the main advertised product. Never return role/tool. Return results only through resultTool, and put the related script IDs in scriptIds.";

/** 新资产：AI 首次识别到的资产，需要完整信息 */
const NewAssetSchema = z.object({
  name: z.string().describe("资产名称,仅为名称不做其他任何表述"),
  desc: z.string().describe("资产描述"),
  type: z.enum(adAssetTypes).describe("资产类型"),
            ...
            messages: [
              {
                ...
                content:
                  ...
                  "\n\nThe input contains multiple ad scripts separated by script IDs. Analyze each script and fill scriptIds for every asset.",
              },
              {
                role: "user",
                content: `当前已有资产列表：${existingHint}\n\n请根据以下${validScripts.length}条广告脚本提取对应的广告资产（产品、人物、场景、道具）:\n\n${scriptsContent}`,
              },
            ],
            tools: { resultTool },
          });
