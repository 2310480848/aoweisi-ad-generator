...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  const projectInfo = [
    "## 项目信息",
    `广告项目名称：${projectData?.name ?? "未知"}`,
    `广告项目类型：${projectData?.type ?? "未知"}`,
    `项目简介：${projectData?.intro ?? "无"}`,
    `目标广告视觉手册|画风：${projectData?.artStyle ?? "无"}`,
    `目标广告视频画幅：${projectData?.videoRatio ?? "16:9"}`,
    `产品资料数量：${novelData.length}份`,
  ].join("\n");
...
function createSubAgent(parentCtx: AgentContext) {
  ...
  const run_sub_agent_adaptationStrategy = tool({
    description: "运行执行subAgent来完成广告创意方案相关任务",
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = path.join(u.getPath("skills"), "script_execution_adaptation.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");

      const formatPrompt = "\n你必须使用如下XML格式写入工作区：\n<adaptationStrategy>广告创意方案内容</adaptationStrategy>";
  ...
  const run_sub_agent_script = tool({
    description: "运行执行subAgent来完成广告脚本相关任务",
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      ...
      const scriptPrompt = ["## 可用广告脚本(ID:名称)", scriptList.map((s: any) => `${s.id}:${(s.name || "").replace(/[,:]/g, "")}`).join(","), ""].join(
        "\n",
      );

      const novelData = await u.db("o_novel").where("projectId", resTool.data.projectId).select("chapterIndex");

      const formatPrompt = `\n你必须使用如下XML格式写入工作区：\nXML不得添加任何额外标签<scriptItem name="广告脚本名称">广告脚本内容</scriptItem><scriptItem name="广告脚本名称">广告脚本内容</scriptItem><scriptItem name="广告脚本名称">广告脚本内容</scriptItem>`;
      ...
      return runAgent({
        ...
        system: systemPrompt + formatPrompt,
        messages: [
          { role: "assistant", content: scriptPrompt + `产品资料数量：${novelData.length}份` },
