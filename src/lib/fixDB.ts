...
export default async (knex: Knex): Promise<void> => {
  ...
  if ((await knex.schema.hasTable("o_agentDeploy")) && (await knex.schema.hasColumn("o_agentDeploy", "vendorId"))) {
    await u.db("o_agentDeploy").whereNot("vendorId", "lingkeai").update({ model: "", modelName: "", vendorId: null });
  }
  if (await knex.schema.hasTable("o_vendorConfig")) {
    await u.db("o_vendorConfig").whereNot("id", "lingkeai").delete();
  }
  //检测是否包含新增音色绑定提示词
  ...
  const baseAgentList = [
    ...
    },
    {
      ...
      oldNames: ["通用AI"],
      oldDescIncludes: ["小说事件提取"],
    },
  ];
  for (const agent of baseAgentList) {
    const exists = await db("o_agentDeploy").where("key", agent.key).first();
    if (!exists) continue;
    const shouldUpdate = !exists.name || !exists.desc || agent.oldNames.includes(exists.name) || agent.oldDescIncludes?.some((text) => exists.desc?.includes(text));
    if (shouldUpdate) {
      await db("o_agentDeploy").where("key", agent.key).update({
        name: agent.name,
        desc: agent.desc,
      });
    }
  ...
  const advancedAgentList = [
    { key: "scriptAgent:decisionAgent", name: "广告脚本Agent:决策层", desc: "决策层" },
    { key: "scriptAgent:supervisionAgent", name: "广告脚本Agent:监督层", desc: "监督层" },
    { key: "scriptAgent:storySkeletonAgent", name: "广告脚本Agent:广告策略框架", desc: "广告策略框架生成" },
    { key: "scriptAgent:adaptationStrategyAgent", name: "广告脚本Agent:广告创意方案", desc: "广告创意方案生成" },
    { key: "scriptAgent:scriptAgent", name: "广告脚本Agent:广告脚本生成", desc: "广告脚本生成" },
  ...
  for (const agent of advancedAgentList) {
    ...
    } else if (!exists.name || !exists.desc || exists.name.startsWith("剧本Agent:")) {
      await db("o_agentDeploy").where("key", agent.key).update({
        name: agent.name,
        desc: agent.desc,
      });
  ...
  await db("o_prompt").where("type", "eventExtraction").update({
    name: "卖点提取",
  ...
  //迁移供应商函数
  const vendorData = readDefaultVendorData();
  const rootDir = u.getPath("vendor");
  if (!fs.existsSync(rootDir)) fs.mkdirSync(rootDir, { recursive: true });
  for (const filename of fs.readdirSync(rootDir)) {
    if (filename.endsWith(".ts") && filename !== "lingkeai.ts") {
      fs.rmSync(path.join(rootDir, filename), { force: true });
    }
  }
  const lingkeAITsCode = vendorData["lingkeai.ts"];
  if (lingkeAITsCode) await tempOnsert(lingkeAITsCode);
...
function readDefaultVendorData(): Record<string, string> {
  const bundledDataPath = (process as any).resourcesPath ? path.join((process as any).resourcesPath, "data", "vendor", "vendor.json") : "";
  const candidatePaths = [bundledDataPath, path.join(process.cwd(), "data", "vendor", "vendor.json"), path.join(u.getPath("vendor"), "vendor.json")].filter(Boolean);

  for (const vendorJsonPath of candidatePaths) {
    if (!fs.existsSync(vendorJsonPath)) continue;
    const data = JSON.parse(fs.readFileSync(vendorJsonPath, "utf8")) as Record<string, string>;
    if (data["lingkeai.ts"]) return { "lingkeai.ts": data["lingkeai.ts"] };
  }
  return {};
}
