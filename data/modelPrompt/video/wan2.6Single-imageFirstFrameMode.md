...
- `id`：资产唯一标识（如 `A001`）
- `type`：资产类型，取值 `product`（产品）/ `person`（人物）/ `scene`（场景）/ `prop`（道具）
...
```xml
<storyboardItem
  videoDesc='（画面描述、场景、关联资产名称、时长、景别、运镜、人物/产品动作、情绪、光影氛围、台词、音效、关联资产ID）'
  prompt='待生成'
  track='分组'
  duration='视频推荐时间'
...
| 10 | 台词 | 台词/音频段 |
| 11 | 音效 | 音效描写 |
| 12 | 关联资产ID | 资产ID↔人物标签映射 |

### 4. 通用约束

- **视觉风格**：风格相关描述参考 Assistant 中的「视觉风格约束」部分内容，不在本 Skill 内自行定义风格
- **仅输出视频提示词**：不附加任何解释、注释、分析过程、推理步骤、分隔线（`---`）或额外说明
- **严格遵循 videoDesc**：提示词内容严格基于 videoDesc 中的画面描述、时长、景别、运镜、人物/产品动作、情绪、光影氛围、台词、音效字段生成，不编造额外内容
...
- **叙事式英文提示词**：像写广告素材一样描写画面，禁止标签罗列（不写 `4K, cinematic, high quality` 这类堆砌）
...
资产信息[A001, product, LingkeAI 冲牙器], [A003, scene, 浴室台面]

```xml
<storyboardItem videoDesc='（LingkeAI 冲牙器独立浴室台面远眺干净镜面和台面、浴室台面、LingkeAI 冲牙器/浴室台面、4s、全景、静止、手持产品展示喷头和水箱细节、清爽可信、明亮柔和侧光、无台词、水流声和轻微环境声、A001/A003）' shouldGenerateImage="true"></storyboardItem>
```
...
资产信息[A001, product, LingkeAI 冲牙器], [A002, person, 使用者], [A003, scene, 浴室台面]

```xml
<storyboardItem videoDesc='（使用者登上浴室台面走向LingkeAI 冲牙器、浴室台面、使用者/LingkeAI 冲牙器/浴室台面、4s、中景、跟踪、使用者走到台面前拿起LingkeAI 冲牙器、好奇、浴室灯光稳定明亮、使用者说：饭后清洁更方便、脚步声和水流声、A001/A002/A003）' shouldGenerateImage="true"></storyboardItem>
```
