...
本项目为广告生成项目。资产类型以 `product`（产品）、`person`（人物/使用者/模特）、`scene`（场景）、`prop`（辅助道具）、`clip`（视频片段）、`audio`（音频）为准；旧写法 `role` 按 `person` 理解，`tool` 按 `prop` 或产品语义理解。分镜表必须围绕产品稳定展示、真实使用过程、卖点证据、人物反馈和转化意图组织，不得把短剧/小说剧情套路带入广告分镜。
...
粒度：一个独立画面 = 一条分镜，约每 50~100 字广告脚本对应 1~2 条分镜。过渡/转场如有明确描写也单独拆分。
...
- **适用场景**：角色/代言人行走穿越空间、跟随动作从A点到B点、环绕角色/代言人展示环境、定场缓推到主体特写等
...
**黄金 6 秒规则**：无文案/台词镜头累计超过 6s 未出现新信息（文案/台词/动作/主体变化），观众注意力断裂。定场+过渡类镜头尤其注意，宁可合并压缩也不要拖沓

---

## 视觉连续性铁律（分镜设计时全程遵守）

**① 动作连续性**：相邻镜头间角色/代言人的位置、动作进度、朝向必须物理逻辑一致。上一镜手伸到半空→下一镜必须从半空状态接续，不能突然收回。
...
**③ 视轴守恒**：180度线原则——对话/对峙场景中角色/代言人画面位置全片固定同侧，不得跳轴
...
- 1 个物理动作 = 1 拍，1 次运镜 = 1 拍，1 句短文案/台词（≤10 字）= 1 拍
- 2~3s 镜头：最多 1 拍；4~6s 镜头：最多 2 拍；7s+ 镜头：最多 3 拍

**⑦ 头尾安全区**：每镜的前 0.5s 和后 0.5s 为安全过渡区，不放关键动作或文案/台词起始点。前 0.5s 用于环境建立或主体静态亮相，后 0.5s 用于动作自然收住。
...
| 全景 | 人物全身与环境 | 角色/代言人登场 / 全身亮相 |
| 中景 | 膝盖以上 | 日常叙事 / 对话 |
| 近景 | 胸部以上 | 情感传达 / 对话重点 |
| 特写 | 面部或物件局部 | 情绪强化 / 关键产品/道具 |
...
**action**（角色/代言人动作）：画面中角色/代言人/主体的具体动作描述（5~40 字），无角色/代言人动作时填 `空镜`。格式为 `(衔接说明)动作描述`。要求：
- **衔接说明置于开头**：用半角括号包裹，置于动作描述最前面。首镜写 `(开篇)`；其他镜头写 `(承接上镜:衔接动作)`，如 `(承接上镜:缓推落幅~群像定格)`、`(承接上镜:手臂半抬状态→继续上扬)`
- **动作链写法**：写连续物理动作链 + 速度节奏（"缓缓抬起右手→指尖微颤→猛然握拳"），禁止只写静态终态。多角色/代言人时各自动作用 `;` 分隔，按关联资产名称顺序排列，如 `黎雾右手摩挲袖口→左臂将兔子玩偶往怀里拢;聂薇视线紧锁兔子方向`
- **本列不再写朝向/空间关系**：朝向与空间关系已拆为独立列（`orientation` / `spatialRelation`），不在 action 内重复标注，避免 `|` 与 markdown 表格列分隔符冲突

**orientation**（朝向）：独立列，画面中角色/代言人面部朝向标注。格式：
- 多角色/代言人按 `associateAssetsNames` 顺序列出，用 `;` 分隔：`角色/代言人A-3/4正面朝右;角色/代言人B-3/4正面朝左`
- 单角色/代言人可省略角色/代言人名：`面朝右`
- 空镜与纯物件特写填 `—`
- 朝向须符合 180° 视轴线规则（同场景内锁定，变化须在 `action` 中给出转身/转头衔接动作并同步更新本列），具体取值见下方朝向参考表

**spatialRelation**（空间关系）：独立列，多角色/代言人画面中各角色/代言人相对站位。格式：
- 按 `associateAssetsNames` 顺序列出，用 `、` 分隔：`角色/代言人A(位置)、角色/代言人B(位置)`
- 位置取值见下方空间关系参考表（9 站位）
- 单角色/代言人镜头可只填一项 `角色/代言人(位置)` 或填 `—`；纯物件特写、空镜填 `—`
- 须与朝向、景别、运镜自洽（朝右的角色/代言人其注视/互动目标应位于其右侧站位）；同场同组角色/代言人站位须稳定，走位须在 `action` 中给出衔接动作并同步更新本列
...
| 朝向取值 | 含义 | 典型场景 |
|---------|------|---------|
| 面朝右 | 水平面朝画面右侧 | 180°线左侧角色/代言人、朝右侧目标 |
| 面朝左 | 水平面朝画面左侧 | 180°线右侧角色/代言人、朝左侧目标 |
| 正面 | 正对镜头 | 自白、宣言、直视观众 |
| 3/4正面朝右 | 3/4侧面偏右朝镜头 | 对话主体（画面偏左角色/代言人） |
| 3/4正面朝左 | 3/4侧面偏左朝镜头 | 对话主体（画面偏右角色/代言人） |
| 正侧面朝右 | 正侧面轮廓朝右 | 独白、沉思 |
| 正侧面朝左 | 正侧面轮廓朝左 | 独白、沉思 |
...
**空间关系参考表**（spatialRelation 列填写用，多角色/代言人场景必标）：
...
| 中前 | 画面中央、靠近镜头 | 单主体居中、被前景半身遮挡的角色/代言人 |
| 右前 | 画面右侧、靠近镜头 | 主体偏右前景 |
| 左中 | 画面左侧、中景层 | 群像中段左位 |
...
**scene**：该分镜所处的场景名称，与广告脚本中的场景对应

**associateAssetsNames**：画面中**可见的**资产名称列表（包括仅局部出现的角色/代言人/物件），便于直观确认关联内容

**duration**：基础参考——特写/表情 2~3s · 对话近景 3~5s · 全身亮相 3~5s · 动作 2~4s · 远景/空镜/过渡 3~5s · 复杂场景 5~8s。**单镜不超过 8s**，超过须拆分。

**含文案/台词时，时长必须足够念完全部文案/台词且匹配情绪语速**：
...
计算方式：文案/台词字数 ÷ 对应语速（向上取整）= 基础秒数，再叠加停顿余量：
- 文案/台词中每个标点停顿（逗号、句号、省略号、破折号等）+0.3~0.5s
- 情绪转折/语气变化处 +0.5s
- 最终 `duration` = 基础秒数 + 停顿累计 + 1s 安全余量（向上取整）

**lines**：角色/代言人文案/台词原文，**必须一字不改从广告脚本中照搬**。多角色/代言人按 `角色/代言人名：文案/台词` 格式排列。无文案/台词填 `无文案/台词`。一句文案/台词对应一个镜头，避免单镜头内塞多角色/代言人多轮对白。

**sound**（音效）：纯音效描述，按「环境音层 + 动作音层」分层。如"远处风声呼啸 + 剑鸣声"。无音效填 `无音效`。

> **🚫 严禁音乐/配乐**：本流水线最终产物**完全不含背景音乐**。`音效` 列只承载真实声源（环境音 + 动作音 + 拟音），任何"BGM""配乐""旋律""管弦/钢琴/竖琴/笛声等乐器作为氛围烘托"等字样**一律违规**，审核会判严重问题。如广告脚本中出现乐器演奏作为广告内容动作（如角色/代言人弹琴），仅可写"指尖拨弦的金属振动声 + 共鸣箱嗡鸣"等具体物理音源。

**associateAssetsIds**：画面中**可见的**资产的 ID（从 assets 数据中获取的实际 `id` 字段值），不编造不存在的 ID。
- **角色/代言人出现即引用**：画面中出现的所有角色/代言人，无论是主体还是仅局部可见（如背影、手部、虚化剪影等），只要在画面内可被辨识，都必须引用其对应的资产 ID
- **场景资产必选**：每条分镜必须引用其所处场景对应的场景资产 ID（type 为 scene 的资产）；若该场景存在匹配当前画面状态的衍生场景资产，则选用衍生场景资产 ID，否则选用主场景资产 ID。缺少场景资产 ID 视为字段不完整
- 父子资产选择规则：按广告内容画面所需状态选择资产 ID——若该镜头需要某主资产的衍生状态，**只选衍生资产 ID**；仅当不存在匹配的衍生状态时，才选择主资产 ID；同一父资产在同一分镜中禁止主/衍生同时出现
