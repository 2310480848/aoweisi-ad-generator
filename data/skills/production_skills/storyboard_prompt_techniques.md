...
> ⚠️ **不生成文生图模式提示词**，所有输出均基于**参考图（图生图 / ControlNet / 角色/代言人一致性）**工作流前提。

本项目为广告生成项目。资产类型以 `product`（产品）、`person`（人物/使用者/模特）、`scene`（场景）、`prop`（辅助道具）、`clip`（视频片段）、`audio`（音频）为准；旧写法 `role` 按 `person` 理解，`tool` 按 `prop` 或产品语义理解。提示词必须优先服务产品稳定展示、真实使用过程、卖点证据和人物反馈，不得编造输入中没有的功效、价格、优惠、实验数据或品牌承诺。
...
| 角色/代言人动作 | 主体体态与朝向 | 动作语义是否一致、朝向是否显式标注 |
| 情绪 | 情绪面容词 | 情绪基调是否一致 |
...
| 画面描述类型 | 处理方式 | 示例 |
|-------------|---------|------|
| **静态瞬间**（停步仰望、站立凝视、侧头冷笑、伏案书写） | **直接按描述生成**，不做动作改写 | "角色/代言人停步仰望某物" → 提示词直接写"停步仰望某物" |
...
| 角色/代言人动作 | 基于分镜表「角色/代言人动作」字段，按"首帧识别原则"处理。必须保留动作语义内涵与 `｜朝向：` 显式标注 |
...
- 除提示词正文外，以下内容默认不输出：分镜标题、参考图绑定说明、文案/台词备注、音效备注、约束检查、资产汇总
...
```
【画面】→ 承载分镜表「画面描述」+「场景」+「景别」+「角色/代言人动作」+「情绪」的完整视觉内容（主干，信息密度最高）
...
- 【画面】段中，角色/代言人/场景/产品/道具名称**必须使用 `@图N` 替代**（不用文字名称）
...
机制：参考图与 prompt 一起作为多模态输入，prompt 使用结构化 JSON 约束角色/代言人一致性。
...
```json
{
  ...
  "shot": {
    ...
    "subject_and_action": "[主体动作 + 朝向 + 情绪 + 所有画面描述中的视觉元素，使用@图N替代角色/代言人/场景名]",
...
- `subject_and_action` 是信息密度最高的字段，须完整承载分镜表「画面描述」+「角色/代言人动作」+「情绪」
- 参考图作为图片输入，不是 URL 文本
- 角色/代言人描述保持 1-2 句关键特征，避免冗长
...
- **画内文字**（场景中自然存在的文字产品/道具：角色/代言人提笔写字、书卷上的字迹、匾额牌匾、书信内容、路标、店铺招牌等）→ **属于场景产品/道具**，当分镜画面描述中明确包含此类内容时，应在【画面】段正常描述其存在，不受禁止文字规则限制
- **判断标准**：该文字是否存在于**故事世界内部**。匾额上的字 = 画内产品/道具 ✅；画面底部的角色/代言人对白 = 画外字幕 ❌
...
5. 不追加任何非提示词区块（如资产引用汇总、文案/台词/音效备注、约束检查）

---

## 图像资产标注规则

每条分镜的 `prompt` 字段必须以**图像资产标注**作为前缀，且**提示词正文中使用 `@图N` 直接替代对应的角色/代言人/场景/产品/道具名称**，建立参考图与画面描述的直接绑定关系。标注按 `associateAssetsIds` 中资产的引用顺序，从 `@图1` 开始依次编号。

**格式**：`@图1 为{资产名称}{资产类型} @图2 为{资产名称}{资产类型} ... , 正文中使用@图N替代角色/代言人/场景名称的提示词`
...
- **正文绑定（核心）**：提示词正文中，所有原本应出现角色/代言人名/场景名/产品/道具名的位置，**必须替换为对应的 `@图N` 标记**，不再使用文字名称。这样参考图与画面中的视觉主体形成直接指向关系，避免资产名称与角色/代言人名称不一致导致的歧义（例如：当衍生资产的名称与原角色/代言人名不一致时，使用 `@图N` 可绕开名称歧义直接指向参考图）
- 同一 `@图N` 在正文中可多次出现（如角色/代言人在前景和反射面中同时可见时）

**示例**（假设 `associateAssetsIds="[A, B, C]"` 对应 角色/代言人甲(role)、角色/代言人乙(role)、某场景(scene)）：

❌ 错误（正文使用文字名称，与前缀标注脱节）：
```
@图1 为角色/代言人甲角色/代言人 @图2 为角色/代言人乙角色/代言人 @图3 为某场景场景, 角色/代言人甲冷笑，居高临下看着跪地的角色/代言人乙，场景内柱影深沉……
```

✅ 正确（正文使用 @图N 直接绑定参考图）：
```
@图1 为角色/代言人甲角色/代言人 @图2 为角色/代言人乙角色/代言人 @图3 为某场景场景,
...
分镜表的「角色/代言人动作」字段已包含 `｜朝向：` 显式标注，提示词生成时**优先直接提取**，并在 prompt 中**显式写入**对应朝向方位词（如 `facing right` / `面朝右`、`three-quarter view facing left` / `3/4侧面朝左`）。

**获取优先级**（高→低）：

| 优先级 | 线索来源 | 处理逻辑 |
|--------|---------|----------|
| **1** | **角色/代言人动作字段的 `｜朝向：` 标注** | 分镜表已显式标注 → **直接采用**，无需推断 |
| 2 | **画面描述中的显式方位词** | 画面描述直接提及朝向（如"背对镜头""望向窗外""面朝观众"）→ 直接采用（仅当优先级1缺失时） |
| 3 | **多角色/代言人空间关系（180° 视轴线）** | 对话/对峙/互动场景中，两角色/代言人面朝彼此：画面左侧角色/代言人面朝右，画面右侧角色/代言人面朝左。首次出场建立基准后全场景锁定 |
...
**获取步骤**：
1. 读取分镜表当前行「角色/代言人动作」字段中 `｜朝向：` 后的标注内容
2. 若标注存在且完整 → 直接采用，跳过后续优先级
3. 若标注缺失（如空镜行）→ 按优先级2~6逐条推断
4. 将获取到的朝向信息写入 prompt 中对应角色/代言人的描述位置
...
| 面朝左 | 面朝画面左侧 | facing left | 180°线右侧角色/代言人、朝左侧目标 |
| 面朝右 | 面朝画面右侧 | facing right | 180°线左侧角色/代言人、朝右侧目标 |
...
- **画面位置锁定**：同一角色/代言人在同一场景内的多条分镜中，其画面左右位置（画面左侧 / 中央 / 右侧）须保持固定，不得无叙事理由地跳侧
- **朝向守恒**：对话/对峙场景遵循 180° 视轴线——角色/代言人A面朝右则全场景保持面朝右，角色/代言人B面朝左则全场景保持面朝左；prompt 中须通过方位词（facing left / 面朝左、on the left side of frame / 画面左侧等）显式标注
- **前后景层次一致**：若角色/代言人A在分镜N中处于前景、角色/代言人B处于中景，则同场景后续分镜中二者前后关系不应无理由反转
- **位置变化须有动作衔接**：角色/代言人画面位置确需变化时（如角色/代言人走动、转身），前序分镜的 prompt 中须包含对应位移/转身动作描写，不可凭空跳位
- **朝向变化须有动作衔接**：角色/代言人朝向确需变化时（如转头、回身），当前分镜的 prompt 中须包含转向动作描写（如"微微转头朝向画面左侧"），且该转向须与分镜表「角色/代言人动作」字段一致，不可凭空改向
...
- **镜像翻转**：反射面中角色/代言人的左右朝向与实体相反（实体面朝右→镜像面朝左），prompt 中须显式标注反射体与实体的朝向关系（如"@图1 面朝右，水面倒影中@图1 面朝左"）
- **反射面不改变位置基准**：角色/代言人的画面位置以实体为准，反射面中的映像不视为角色/代言人位置变化
- **反射面内容与实体一致**：反射面中可见的角色/代言人服饰、发型、表情等必须与同帧实体一致，不可出现偏差
...
以下示范一条分镜从输入到输出的完整流程，供 Agent 参考。本示例使用抽象占位符（角色/代言人甲、某场景、产品/道具X 等），实际应用时替换为分镜表的具体内容。

### 输入（分镜表某行）

| 字段 | 内容 |
|------|------|
| 画面描述 | 开场黑场淡入，某场景出口大远景，人流涌动，醒目指示物立于画面右侧，角色/代言人甲背着产品/道具X 独行人流中，镜头缓推至中景，他手攥产品/道具Y 忽然停步仰望指示物，眼神紧张而笃定 |
| 场景 | 某场景出口 |
| 景别 | 远景→中景 |
| 角色/代言人动作 | 背包步行前行→忽然停步→抬头仰望指示物→手攥产品/道具Y 微紧｜朝向：3/4正面朝右 |
| 情绪 | 局促与笃定并存 |
| 光影氛围 | 左侧柔和晨光均匀铺洒，暖黄底色轻染地面，指示物受光清晰，人影逆光偏暗形成轮廓感 |
| 关联资产ID | [a, b, c, d] → 角色/代言人甲(role)、产品/道具X(tool)、产品/道具Y(tool)、某场景出口(scene) |

### 输出（模式A · Seedream）

```
@图1 为角色/代言人甲角色/代言人 @图2 为产品/道具X 产品/道具 @图3 为产品/道具Y 产品/道具 @图4 为某场景出口场景,
...
| 角色/代言人甲背产品/道具X 独行 | 【画面】"@图1 背着@图2 独行于人流之中" | ✅ |
| 手攥产品/道具Y | 【画面】"手中紧攥@图3" | ✅ |
